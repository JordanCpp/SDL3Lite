...
#include <SDL3/Renders/OpenGL1/GL1Rndr.hpp>
#include <SDL3/SoftRndr.hpp>
#include <SDL3/SurfRndr.hpp>
#include <SDL3/Accel.hpp>
...
SDL_Renderer* SDL_CreateRendererImplementation(Result& result, SDL_Window* window, const char* name)
{
	...
	{
		renderer = new OpenGL1Render(result, window);
	}
	else
	{
		renderer = new SoftwareRender(window);
	}
...
{
	return new SurfaceRender((Surface*)surface);
}

SDL_Renderer* SDL_CreateRenderer(SDL_Window *window, const char *name)
{
	return SDL_CreateRendererImplementation(GetApplication().GetResult(), window, name);
}
...
bool SDL_SetRenderDrawColor(SDL_Renderer* renderer, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	renderer->SetColor(Color(r, g, b, a));
...
void SDL_RenderFillRect(SDL_Renderer* renderer, SDL_FRect* rect)
{
	...
	renderer->FillRect(Vec2f(rect->x, rect->y), Vec2f(rect->w, rect->h));
}
...
bool SDL_RenderLine(SDL_Renderer* renderer, float x1, float y1, float x2, float y2)
{
	...
	renderer->Line(Vec2f(x1, y1), Vec2f(x2, y2));
...
bool SDL_RenderTexture(SDL_Renderer* renderer, SDL_Texture* texture, const SDL_FRect* srcrect, const SDL_FRect* dstrect)
{
	...
	if (srcrect == NULL)
	{
		srcRect = Rect2f(0, 0, (float)texture->GetSize().x, (float)texture->GetSize().y);
	}
	else
	{
		srcRect = srcrect;
	...
	if (dstrect == NULL)
	{
		dstRect = Rect2f(0, 0, (float)renderer->GetSize().x, (float)renderer->GetSize().y);
	}
	else
	{
		dstRect = dstrect;
